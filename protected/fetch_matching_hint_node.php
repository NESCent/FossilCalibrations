...
$query="SELECT taxonid AS nameid, 'NCBI' AS source
	FROM NCBI_names
	WHERE name LIKE '". mysql_real_escape_string($_POST["matched_name"]) ."'
     OR uniquename LIKE '". mysql_real_escape_string($_POST["matched_name"]) ."'
    LIMIT 1;";
...
if (!$node_data) {
    ...
    $query="SELECT node_id AS nameid, 'FCD' AS source
        FROM FCD_names
        WHERE name LIKE '". mysql_real_escape_string($_POST["matched_name"]) ."'
     OR uniquename LIKE '". mysql_real_escape_string($_POST["matched_name"]) ."'".
...
<input type="text" readonly="readonly" style="width: 15%; color: #999; text-align: center;" 
       name="hintNodeID_<?= $_POST["side"] ?>[]" id="hintNodeID_<?= $_POST["side"] ?>_<?= $_POST["position"] ?>" value="<?= (!$node_data) ? 'ERROR' : $node_data['nameid'] ?>" />
